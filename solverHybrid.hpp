				...
				switch(Tag)
				{
					case PBM_TAG:
						...
						while(!bqueue.empty()) 
						{
							...
							if(nfslaves == 0)
							{
								int nt = bqueue.size() > MAXTHREADS ? MAXTHREADS:bqueue.size();
								for(int i=0;i<nt;i++)
								{
									data[i] = bqueue.front();
									bqueue.pop();
								...
								for(int i=0;i<nt;i++)
								{
									if(this->probType == MAX)
									{
										if(data[i].feasibility(pbm)==1)
										{
											#pragma omp critical
											{
												if(bestSol < data[i].getSolutionMeasure(pbm))
												{
													bestSol = data[i].getSolutionMeasure(pbm);
													bestTempSol = data[i].sol;
													temp = (float)data[i].getSolutionMeasure(pbm);
													flag=1;
													
													/*MPI_Send(&dummy, 1, MPI_INT, 0, 
														SOLVE_TAG, MPI_COMM_WORLD);
													//printf("%d => after send1\n", omp_get_thread_num());

													Bound temp = (float)data[i].getSolutionMeasure(pbm);
													
													//printf("Thread solved %d, %f\n",omp_get_thread_num(),temp);
													
													MPI_Send(&temp, sizeof(temp), MPI_BYTE, 0, 
														SOLVE_TAG, MPI_COMM_WORLD);
													//printf("%d => after send2\n", omp_get_thread_num());

													MPI_Send(&subp.sol, sizeof(subp.sol), MPI_BYTE, 0, 
														SOLVE_TAG, MPI_COMM_WORLD);
													//printf("%d => after send3\n", omp_get_thread_num());
													*/
												}
											}
								...
								if(flag==1)
								{
									...
									MPI_Send(&bestTempSol, sizeof(bestTempSol), MPI_BYTE, 0, 
										SOLVE_TAG, MPI_COMM_WORLD);
								}
